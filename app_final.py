...
def forgot_password_simulation(email_or_username, is_admin=False):
    ...
    time.sleep(1)
    if is_admin:
        st.sidebar.success(f" Yönetici Şifresi sıfırlama maili 'admin@portfolyo.com' adresine gönderildi.") # F-string hatası düzeltildi
    else:
...
if st.session_state['music_enabled'] and st.session_state['music_url']:
    # PARAMETRELER BASİTLEŞTİRİLDİ (TypeError hatasını çözmek için 'html_attrs' kaldırıldı)
    st.audio(
