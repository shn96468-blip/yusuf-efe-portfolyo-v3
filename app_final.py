...
DEFAULT_DERSLER = {
    ...
    "Sosyal Bilgiler": {
        ...
        "sorular": [
            {"q": " Aşağıdakilerden hangisi etkili iletişimi olumsuz etkiler?", "a": ["Empati kurmak", "Göz teması kurmak", "Yargılayıcı konuşmak", "Açık ve net konuşmak"], "c": "Yargılayıcı konuşmak"}, # <<< Hata buradaydı, kapatma tırnağı (") eklendi.
...
# --- DERS MODÜLLERİ (Konu Anlatımı, PDF Kontrol, Deneme) ---
...
def render_dinamik_test_alani(ders_adi, sorular):
    ...
    st.info("Aşağıdaki alana testini çözmek istediğiniz konunun adını yazın ve 'Testi Oluştur' düğmesine basın.")

    # Konu Adı Girişi
    with st.form(f"test_konusu_form_{ders_adi}", clear_on_submit=False):
        
        # Konu Adı Giriniz Alanı (Görsellerdeki gibi)
        st.markdown("##### Test Konusu:")
        ...
        if st.form_submit_button("Testi Oluştur"):
            if test_konusu_input:
                ...
                # Testi oluşturduktan sonra formu kapatıp test içeriğini gösterelim
                st.rerun()
            else:
    ...
    if st.session_state['test_konusu'] and st.session_state['secilen_modul'] == "Deneme Sınavı":
        ...
        with st.form(f"quiz_form_{ders_adi}_soru", clear_on_submit=False): # Key çakışmasını engellemek için key'e _soru ekledim
            kullanici_cevaplari = {}
            
            # Dinamik konu başlığını yansıtmak için soruları yeniden yazarız.
            guncel_sorular = [{"q": q['q'].replace("Tam Sayılarla Dört İşlem", st.session_state['test_konusu']), **q} for q in sorular]
